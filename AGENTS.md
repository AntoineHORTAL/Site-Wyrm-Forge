...
> **Statut : commité, push en attente d'arbitrage** — le déploiement passe par la GitHub Action `deploy-supabase-functions.yml` (déclenchée par `supabase/functions/**` sur `main`). Cette ligne devra passer à « déployé le \<date\> » une fois le push effectué. Scripts de mesure rejouables dans le scratchpad de session (`measure-matchup-cost.mjs`, `measure-prompt-v2.mjs`, `verify-v2-realstats.mjs`, `measure-final.mjs`).
